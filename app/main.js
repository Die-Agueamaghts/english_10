(() => {
  ...
  function normalizeCard(card) {
    ...
    if (normalized.english_word === undefined) {
      normalized.english_word = normalized.english ?? normalized.question ?? "";
    }
  ...
  function renderCard() {
    ...
    if (state.mode === "learn") {
      ...
      renderTextWithPlay(
        ...
        getenglishValue(card),
        "en-US",
        "Englisch",
      );

      const englishExample = getenglishExampleValue(card);
      const germanExample = getGermanExampleValue(card);
      const exampleValue = englishExample || germanExample;
      const exampleLang = englishExample ? "en-US" : "de-DE";
      renderTextWithPlay(
        exampleTextContainer,
        exampleValue,
        exampleLang,
        exampleLang === "en-US" ? "Englisch" : "Deutsch",
      );
    ...
    renderTextWithPlay(
      $("#questionText"),
      promptText,
      isQuestionPrompt ? "en-US" : "de-DE",
      isQuestionPrompt ? "Englisch" : "Deutsch",
    );

    const exampleValue = isQuestionPrompt
      ? getenglishExampleValue(card)
      : getGermanExampleValue(card);
    const exampleLang = isQuestionPrompt ? "en-US" : "de-DE";
    renderTextWithPlay(
      exampleTextContainer,
      exampleValue ||
        (isQuestionPrompt
